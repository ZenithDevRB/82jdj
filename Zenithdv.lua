...
-- Desactivar errores molestos del juego
...
-- Ejecutar protección
pcall(suppressGameErrors)

-- También proteger ScriptContext
local scriptContext = game:GetService("LogService")
scriptContext.MessageOut:Connect(function(message, messageType)
    if messageType == Enum.MessageType.MessageError then
        if message:find("attempt to call a nil value") then
            -- Bloquear estos mensajes de error
            return
        end
    end
end)
...
AttackLoop = RunService.Heartbeat:Connect(function()
    pcall(function()
        ...
        -- Método 1: Buscar en workspace.Live (común en muchos juegos)
        ...
        -- Método 2: Buscar en workspace directamente (backup)
